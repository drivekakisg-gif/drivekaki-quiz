    ...
    <div className="flex gap-2 justify-center">
      {Array.from({ length: 6 }).map((_, i) => (
        <input
          ...
          onPaste={handlePaste}
          className="w-11 h-13 text-center text-xl font-bold border-2 border-gray-200 rounded-xl focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-100 transition-colors"
        />
      ))}
    </div>
  );
...
function PhoneForm() {
  ...
  useEffect(() => {
    if (step === "otp") {
      setTimeout(() => {
        const firstInput = firstOtpRef.current?.querySelector("input");
        firstInput?.focus();
      }, 100);
    }
  }, [step]);
      ...
      <form onSubmit={verifyOtp} className="space-y-5">
        <div className="text-center">
          <p className="text-sm text-gray-500">
            OTP sent to <strong>{COUNTRY_CODE} {phone}</strong>
          </p>
          <button
            type="button"
        ...
        {error && (
          <p className="text-sm text-red-600 bg-red-50 px-4 py-2 rounded-lg text-center">
            {error}
          </p>
        )}
        <button
    ...
    <form onSubmit={sendOtp} className="space-y-4">
      <div>
        ...
        <div className="flex gap-2">
          <div className="flex items-center px-3 py-3 border border-gray-300 rounded-xl bg-gray-50 text-sm font-semibold text-gray-700 shrink-0">
            🇸🇬 {COUNTRY_CODE}
          </div>
          <input
            type="tel"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        <a
          href="https://wa.me/6591234567?text=I%20want%20to%20sign%20up%20for%20DriveKaki%20Theory"
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center justify-center gap-2 py-2.5 rounded-xl border border-gray-200 text-sm font-semibold text-white transition-opacity hover:opacity-90"
        ...
        <a
          href="https://t.me/drivekaki_bot?start=signup"
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center justify-center gap-2 py-2.5 rounded-xl border border-gray-200 text-sm font-semibold text-white transition-opacity hover:opacity-90"
