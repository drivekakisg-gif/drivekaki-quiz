...
export default function AnswerButton({ letter, text, state, onClick }: AnswerButtonProps) {
  ...
  const styles: Record<ButtonState, string> = {
    ...
    correct:  `${base} bg-[#22c55e] border-[#22c55e] text-white`,
  ...
  const badgeStyles: Record<ButtonState, string> = {
    ...
    correct:  "w-8 h-8 rounded-lg bg-white flex items-center justify-center text-xs font-bold text-green-600 flex-shrink-0",
  ...
  return (
    ...
    >
      ...
      {state === "correct" && <span className="flex-shrink-0 text-white text-lg">✓</span>}
